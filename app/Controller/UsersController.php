 ...
 */
    ...
    public function invite() {
        ...
        if ($this->request->is('post')) { // if data was posted therefore a submitted form
            ...
            if ($this->User->save($data)) {
                $id = $this->User->getLastInsertID();
                $this->log("[UsersController.invite] user[${id}] created by user[" . $this->Auth->user('id') . "]", 'sourcekettle');
                ...
                $this->log("[UsersController.invite] user[" . $id . "] added by user[" . $this->Auth->user('id') . "]", 'sourcekettle');
